...
static int wacom_initialize_leds(struct hid_device *hdev)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (ret) {
			wdata->leds[i] = NULL;
			kfree(led);
			dev_warn(hdev, "can't register LED\n");
			goto err;
		}
	}
...
static void wacom_set_features(struct hid_device *hdev, u8 speed)
{
	...
	case USB_DEVICE_ID_WACOM_GRAPHIRE_BLUETOOTH:
		 ...
		 */
		dev_warn(hdev, "failed to poke device, command %d, err %d\n",
			 rep_data[0], ret);
		break;
			...
			struct input_dev *input, unsigned char *data)
{
	...
	case 0x0C: /* Button report */
		...
		break;
	default:
		dev_warn(hdev, "Unknown report: %d,%d\n", data[0], data[1]);
		break;
	}
}
		...
		u8 *raw_data, int size)
{
	...
	case USB_DEVICE_ID_WACOM_GRAPHIRE_BLUETOOTH:
		if (data[0] == 0x03) {
			...
			dev_err(hdev, "Unknown report: %d,%d size:%d\n",
					data[0], data[1], size);
			return 0;
		}
		break;
	case USB_DEVICE_ID_WACOM_INTUOS4_BLUETOOTH:
		...
		case 0x03:
			...
			break;
		default:
			dev_warn(hdev, "Unknown report: %d,%d size:%d\n",
					data[0], data[1], size);
			return 0;
		}
	}
